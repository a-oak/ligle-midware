...
var Renderer = Class.extend({
   ...
   * @return {null}
   */
  renderEO:function(){// jshint ignore:line
    var fileSucc, fileErr,err,obj;
    if(arguments.length===3){
      fileErr = fileSucc = arguments[0];
    }else if(arguments.length===4){
      fileErr = arguments[0];
      fileSucc = arguments[1];
    }else{
    ...
    if(err instanceof Error){
      obj.error=JSON.stringify(err.message);
    }else if(err){
      obj.error=JSON.stringify(err);
    }
    obj = this._wrap(obj);
    if(err){
      return this.res.render(fileErr, obj);
    }else {
      return this.res.render(fileSucc, obj);
    }
  },
  /**
   * 传入错误信息，并回退。
